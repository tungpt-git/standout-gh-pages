...
(function ($) {
  ...
  /* Interested List
   * ------------------------------------------------------ */
  var fetchInterestingList = function () {
    const fetch = () => {
      const list = $("#interested .interested-list");

      const data = Array.from({ length: 10 }).map((_, idx) => ({
        title: "Item " + (idx + 1),
        image: "images/phone-app-screens-2000.png",
      }));

      const toInterestItemStr = (item) => {
        return `<div class="interested-item">
